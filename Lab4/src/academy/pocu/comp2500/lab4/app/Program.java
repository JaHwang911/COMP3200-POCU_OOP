...
import academy.pocu.comp2500.lab4.LinkedMemoryCache;
...
//        testManyEntry();
    ...
    private static void testLinkedMap() {
        {
            LinkedMemoryCache memCacheA = LinkedMemoryCache.getInstance("A");

            LinkedMemoryCache memCacheB = LinkedMemoryCache.getInstance("B");
            LinkedMemoryCache memCacheC = LinkedMemoryCache.getInstance("C");

            assert memCacheA == LinkedMemoryCache.getInstance("A");
            assert memCacheB == LinkedMemoryCache.getInstance("B");
            assert memCacheC == LinkedMemoryCache.getInstance("C");

            LinkedMemoryCache.setMaxInstanceCount(3);

            LinkedMemoryCache memCacheD = LinkedMemoryCache.getInstance("D");

            assert memCacheA != LinkedMemoryCache.getInstance("A");
            assert memCacheC == LinkedMemoryCache.getInstance("C");
            assert memCacheB != LinkedMemoryCache.getInstance("B");
            assert memCacheD != LinkedMemoryCache.getInstance("D");
        }

        {
            LinkedMemoryCache.clear();
            LinkedMemoryCache memCache = LinkedMemoryCache.getInstance("A");
