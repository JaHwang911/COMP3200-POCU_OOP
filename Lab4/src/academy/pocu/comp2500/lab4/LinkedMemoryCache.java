...
public class LinkedMemoryCache {
    private final String name;
    private int maxEntryCount;
    private LinkedMemoryCache previous;
    private LinkedMemoryCache next;
    private LinkedMemoryCache lruPrevious;
    private LinkedMemoryCache lruNext;
    ...
    private static final HashMap<String, LinkedMemoryCache> CACHES = new HashMap<>(256);
    ...
    private LinkedMemoryCache(String name, LinkedMemoryCache lruNextOrNull, LinkedMemoryCache previous) {
        this.name = name;
    ...
    public static LinkedMemoryCache getInstance(String name) {
        if (CACHES.containsKey(name)) {
            LinkedMemoryCache current = CACHES.get(name);
            LinkedMemoryCache previous = current.lruPrevious;
            LinkedMemoryCache next = current.lruNext;
            ...
            LinkedMemoryCache firstCache = CACHES.get(lruFirstCacheName);
        ...
        if (CACHES.size() == 0) {
            CACHES.put(name, new LinkedMemoryCache(name,null, null));
        ...
        LinkedMemoryCache lruFirstCache = CACHES.get(lruFirstCacheName);
        LinkedMemoryCache lastOrderedCache = CACHES.get(lastCacheName);
        LinkedMemoryCache current = new LinkedMemoryCache(name, lruFirstCache, lastOrderedCache);
    ...
    private static void removeCacheByEvictionPolicy() {
        LinkedMemoryCache removedCache;
        LinkedMemoryCache prev;
        LinkedMemoryCache next;
